"""
Zero-shot forecasting baseline with a RANDOM (untrained) encoder.

Frozen random PatchTST backbone + trained PredictionHead.
Used as a control to measure how much pretraining contributes.
"""
...
# ── path setup ────────────────────────────────────────────────────────────────
_RANDOM_DIR = os.path.dirname(os.path.abspath(__file__))
_ROOT_DIR   = os.path.dirname(_RANDOM_DIR)
_NPT_DIR    = os.path.join(_ROOT_DIR, "NPT")
...
def _build_model(config, c_in, forecast_len, device):
    return PatchTST(
        c_in=c_in,
...
def random_forecasting(config, forecast_dataset: str = None):
    """
    Frozen random backbone + trained PredictionHead on forecast_dataset.

    Parameters
    ----------
    config           : dict — uses same keys as config_ntp.py
    forecast_dataset : dataset name (overrides config["forecast_dataset"])
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    forecast_dset = forecast_dataset or config.get("forecast_dataset", "ettm1")

    print(f"\n{'='*60}")
    print("  RANDOM BASELINE — Frozen Random Encoder + Trained Head")
    print(f"  dataset : {forecast_dset}   device : {device}")
    print(f"{'='*60}")
    ...
    model = _build_model(fore_cfg, n_vars, forecast_len, device)
    for param in model.backbone.parameters():
        param.requires_grad = False
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"  Random backbone frozen. Head trainable: {trainable:,}\n")
    ...
    for epoch in range(epochs_head):
        ...
        for context_patches, target_patch in train_loader:
            ...
            x    = context_patches.permute(0, 1, 3, 2)   # [B, P, n_vars, P_L]
    ...
    print(f"\n  [Random Baseline — {forecast_dset}]")
    print(f"  MSE : {mse:.4f}")
    print(f"  MAE : {mae:.4f}")

    # ── plots ─────────────────────────────────────────────────────────────────
    pred_out, target_out = last_batch
    save_dir = os.path.join(_RANDOM_DIR, "output_model", forecast_dset)
    os.makedirs(save_dir, exist_ok=True)

    sample = 0
    for var_idx in range(min(n_v, 3)):
        ...
        plt.plot(pred, label="Random Baseline", color="red",   linestyle="--", alpha=0.9)
        plt.title(f"Random Baseline — {forecast_dset} — Variable {var_idx} ({forecast_len} steps)")
        plt.xlabel("Time Steps")
        plt.ylabel("Value")
        plt.legend()
        plt.grid(True, linestyle="--", alpha=0.5)
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f"var{var_idx}.png"))
        plt.close()
...
if __name__ == "__main__":
    ...
    random_forecasting(dict(_mod.config))
