...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int,
        ...
        assert head_type in ['pretrain', 'prediction', 'regression', 'classification', 'ntp'], \
            'head type should be pretrain, prediction, regression, classification, or ntp'
        # Backbone
        self.backbone = PatchTSTEncoder(c_in, num_patch=num_patch, patch_len=patch_len,
                                ...
                                pe=pe, learn_pe=learn_pe, verbose=verbose, causal=causal, **kwargs)
...
class PatchTSTEncoder(nn.Module):
    def __init__(self, c_in, num_patch, patch_len,
                 ...
                 pe='zeros', learn_pe=True, causal=False, verbose=False, **kwargs):
    ...
    def forward(self, x) -> Tensor:          
        ...
        if self.causal:
            attn_mask = torch.triu(
                torch.ones(num_patch, num_patch, device=x.device, dtype=torch.bool),
                diagonal=1
            )
